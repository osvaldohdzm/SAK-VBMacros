...
Sub CYB008_GenerarPruebasPorServicio_Individual()
    Dim wsCat As Worksheet, wsSur As Worksheet, wsDest As Worksheet
    Dim tblCat As ListObject, tblSur As ListObject, tblDest As ListObject
    Dim filaCat As ListRow, filaSur As ListRow, nuevaFila As ListRow
    Dim ambito As String, servicioBusqueda As String
    Dim confirmacion As VbMsgBoxResult
    Dim contador As Integer: contador = 0
    
    ' 1. Configuración inicial
    On Error Resume Next
    Set tblCat = ActiveSheet.ListObjects("SecEvalCatalog")
    If tblCat Is Nothing Then
        MsgBox "Selecciona una celda dentro del Catálogo de Pruebas.", vbCritical
        Exit Sub
    End If
    On Error GoTo 0

    ' 2. Obtener datos de la fila seleccionada en Catálogo
    Set filaCat = tblCat.ListRows(ActiveCell.row - tblCat.HeaderRowRange.row)
    servicioBusqueda = filaCat.Range(tblCat.ListColumns("Categoría / Servicio").Index)
    ' Limpiar el nombre del servicio (quitar puertos entre paréntesis si existen, ej: "SNMP (161)" -> "SNMP")
    If InStr(servicioBusqueda, "(") > 0 Then
        servicioBusqueda = Trim(Left(servicioBusqueda, InStr(servicioBusqueda, "(") - 1))
    End If

    ' 3. Preguntar Ámbito (Interno o Externo)
    ambito = InputBox("¿Es una prueba INTERNA o EXTERNA? (Escribe 'I' o 'E')", "Definir Ámbito")
    ambito = UCase(ambito)

    If ambito = "I" Then
        Set wsDest = ThisWorkbook.Sheets("Pruebas Internas Seleccionadas")
        Set tblDest = wsDest.ListObjects("TblSecurityTestsSelectedIntern")
        Set tblSur = Range("TblAttackInternalSurface").ListObject
    ElseIf ambito = "E" Then
        Set wsDest = ThisWorkbook.Sheets("Pruebas Seleccionadas") ' Ajustar nombre si es diferente
        Set tblDest = wsDest.ListObjects("TblSecurityTestsSelectedExtern")
        Set tblSur = Range("TblAttackExternalSurface").ListObject
    Else
        MsgBox "Opción no válida. Operación cancelada.", vbExclamation
        Exit Sub
    End If

    ' 4. Recorrer Superficie de Ataque y generar pruebas
    Application.ScreenUpdating = False
    
    For Each filaSur In tblSur.ListRows
        ' Comparar servicio (insensible a mayúsculas/minúsculas)
        If InStr(1, filaSur.Range(tblSur.ListColumns("Servicio").Index), servicioBusqueda, vbTextCompare) > 0 Then
            
            ' Crear nueva fila en la tabla de Pruebas Seleccionadas
            Set nuevaFila = tblDest.ListRows.Add
            contador = contador + 1
            
            With nuevaFila
                .Range(1) = tblDest.ListRows.Count ' Folio auto-incremental
                .Range(2) = Date                   ' Fecha
                .Range(3) = filaCat.Range(tblCat.ListColumns("Fase").Index) ' Fase
                
                ' --- DATOS DEL ACTIVO (IP y Puerto) ---
                .Range(4) = filaSur.Range(tblSur.ListColumns("IP").Index) & " (" & _
                            filaSur.Range(tblSur.ListColumns("Puerto").Index) & ")" ' Activo Tecnológico
                
                .Range(5) = filaCat.Range(tblCat.ListColumns("Descripción").Index) ' Objetivo
                .Range(6) = filaCat.Range(tblCat.ListColumns("Id").Index)          ' ID Catálogo
                .Range(7) = filaSur.Range(tblSur.ListColumns("Servicio").Index)   ' Servicio Evaluado
                .Range(8) = filaCat.Range(tblCat.ListColumns("Actividad de Checklist").Index) ' Nombre Prueba
                .Range(9) = filaCat.Range(tblCat.ListColumns("Descripción").Index) ' Descripción
                
                ' Evidencia Principal (Ruta base del catálogo)
                .Range(11) = filaCat.Range(tblCat.ListColumns("Evidencia Principal").Index)
            End With
        End If
    Next filaSur
    ...
    ' 5. Resultado
    If contador > 0 Then
        MsgBox "Se han generado " & contador & " pruebas basadas en los activos encontrados para: " & servicioBusqueda, vbInformation, "Proceso Completado"
        wsDest.Activate
    Else
        MsgBox "No se encontraron activos con el servicio: " & servicioBusqueda & " en la superficie de ataque seleccionada.", vbExclamation
    End If
            ...
            If Not tblSurInt Is Nothing Then
                For Each filaSur In tblSurInt.ListRows
                    ...
                    If MatchServicios(servCat, servSur) Then
                        ...
                        Call RellenarLineaComandos(tblDestInt, filaCat, filaSur, tblCat, tblSurInt, contadorInt)
                    End If
                Next filaSur
            End If
...
Function MatchServicios(ByVal Cat As String, ByVal Sur As String) As Boolean
    ...
    Select Case True
        ...
        Case Cat = "SSH" And Sur = "22"
            MatchServicios = True
        Case Cat = "RDP" And (Sur = "3389" Or InStr(Sur, "MS-WBT") > 0)
            MatchServicios = True
