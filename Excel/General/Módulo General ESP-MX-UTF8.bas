...
Sub GEN012_VaciarTabla()
    ...
    If Not tbl Is Nothing Then
        ' Confirmaci�n antes de eliminar las filas
        If MsgBox("�Est� seguro de que desea eliminar todas las filas de la tabla?", vbYesNo + vbExclamation, "Confirmaci�n") = vbYes Then
        ...
        End If
    Else
        MsgBox "La celda seleccionada no est� dentro de una tabla.", vbCritical, "Error"
    End If
...
Sub GEN013_VaciarTablaAFilaEjemplo()
    ...
    If Not tbl Is Nothing Then
        ' Confirmaci�n antes de eliminar las filas
        If MsgBox("�Est� seguro de que desea eliminar todas las filas excepto la primera?", vbYesNo + vbExclamation, "Confirmaci�n") = vbYes Then
...
Sub GEN014_VaciarTodasTablasAFilaEjemplo()
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        For Each tbl In ws.ListObjects
            ' Verificar si la tabla est� en la lista de exclusi�n
            If Not excludeTables.exists(tbl.Name) Then
                ' Verificar que la tabla tenga m�s de una fila
                If tbl.ListRows.Count > 1 Then
                    ' Eliminar todas las filas excepto la primera
                    On Error Resume Next
                    tbl.DataBodyRange.Offset(1, 0).Resize(tbl.ListRows.Count - 1, tbl.ListColumns.Count).Delete
